			...
			<van-tabs>
				<van-tab title="基本信息">
					...
					<van-cell-group>
						...
						<van-field id="JOB_JSYDCQXXB.FQTYY" v-model="qtyy" clearable placeholder="其他原因" />
					</van-cell-group>
					<van-cell-group>
						<div class="cell-title">
							备注
						</div>
						<van-field id="JOB_JSYDCQXXB.FBZ" v-model="bz" clearable placeholder="备注" />
					</van-cell-group>
				</van-tab>
				<van-tab title="权利人">
					<van-cell-group>
	...
	export default {
		...
		methods: {
			...
			onSelect: function (val) {
				if (this.optionsTitle === 'itemOptions') { // 申请事项
					...
					this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBDCLX'] = val.name;
				} else if (this.optionsTitle === 'reasonOptions') { // 登记原因
					this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FDJYY'] = val.name;
			...
			nextStep: function () {
				...
				if (!this.$data['JOB_BDCQK']['JOB_SJDJB.FDJLX'] || this.$data['JOB_BDCQK']['JOB_SJDJB.FDJLX'].length ==
					...
					Toast('请选择镇区!');
					return;
				} else if (!this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBDCDYH'] || this.$data['JOB_BDCQK'][
						'JOB_JSYDCQXXB.FBDCDYH'
					].length == 0) {
					Toast('请填写不动产单元号!');
					return;
				} else if (!this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBDCLX'] || this.$data['JOB_BDCQK'][
						'JOB_JSYDCQXXB.FBDCLX'
					].length == 0) {
					Toast('请选择不动产类型!');
					return;
				} else if (!this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FZL'] || this.$data['JOB_BDCQK'][
						'JOB_JSYDCQXXB.FZL'
					].length == 0) {
					Toast('请填写坐落!');
					return;
				} else if (!this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBDCQZH'] || this.$data['JOB_BDCQK'][
						'JOB_JSYDCQXXB.FBDCQZH'
					].length == 0) {
					Toast('请填写原不动产权证号!');
					return;
				} else if (!this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FDJYY'] || this.$data['JOB_BDCQK'][
						'JOB_JSYDCQXXB.FDJYY'
					].length == 0) {
					Toast('请选择登记原因!');
					return;
			...
			startExactBusiness: function (rid, businessNumber) {
				...
				this.$fetch(START_EXACT_BUSINNESS, {
						...
						_this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBDCLX'] = bdclx;
						_this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FQTYY'] = '';
						_this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBZ'] = '';
						...
						_this.applicantIndex = 0;
						// _this.$data['JOB_SQRXXB_LINK.IQLR']['JOB_JSYDCQXXB.FBDCLX'] = bdclx;
		...
		created() {
			...
			if (formdata) { // 不过本地存在业务则不创建
				...
				this.qtyy = this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FQTYY'];
				this.bz = this.$data['JOB_BDCQK']['JOB_JSYDCQXXB.FBZ'];
