					...
					<van-tab title="基本信息">
						...
						<van-cell-group>
							...
							<van-field v-model="valuesParams['JOB_FDCQXXB.FFWXZ']" right-icon="arrow" placeholder="请选择房屋性质"
								clickable @click.native="housePropertyClicked()" />
						</van-cell-group>
						<van-cell-group>
							<div class="cell-title">
							...
							<van-field v-model="valuesParams['JOB_FDCQXXB.FFWYT']" right-icon="arrow" placeholder="请选择房屋用途"
								clickable @click.native="houseUseClicked()" />
						</van-cell-group>
						<van-cell-group>
							<div class="cell-title">
							...
							<div class="unFillText">
								<van-field style="flex:80%" class="field-background"
									v-model="valuesParams['JOB_FDCQXXB.FZDMJ']" />
						...
						<van-cell-group>
							...
							<van-field v-model="valuesParams['JOB_FDCQXXB.FFWLX']" right-icon="arrow" placeholder="请选择房屋类型"
								clickable @click.native="houseTypeClicked()" />
						</van-cell-group>
						<van-cell-group>
							<div class="cell-title">
								<span class="required-span">*</span>总层数
							</div>
							<van-field v-model="valuesParams['JOB_FDCQXXB.FZCS']" clearable placeholder="总层数" />
						</van-cell-group>
						<van-cell-group>
							<div class="cell-title">
								<span class="required-span">*</span>终止层
							</div>
							<van-field v-model="valuesParams['JOB_FDCQXXB.FZZC']" clearable placeholder="终止层" />
						</van-cell-group>
					</van-tab>
					<van-tab title="申请人">
						<van-cell-group>
			...
			methods: {
				...
				nextStep: function () {
					...
					if (!this.valuesParams['JOB_SJDJB.FDJLX'] || this.valuesParams['JOB_SJDJB.FDJLX'].length == 0) {
						...
						Toast('请填写坐落!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FQLXZ'] || this.valuesParams['JOB_FDCQXXB.FQLXZ'].length ==
						0) {
						Toast('请填写土地性质!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FTDYT'] || this.valuesParams['JOB_FDCQXXB.FTDYT'].length ==
						0) {
						Toast('请填写土地用途!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FFWXZ'] || this.valuesParams['JOB_FDCQXXB.FFWXZ'].length ==
						0) {
						Toast('请选择房屋性质!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FFWYT'] || this.valuesParams['JOB_FDCQXXB.FFWYT'].length ==
						0) {
						Toast('请选择房屋用途!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FZDMJ'] || this.valuesParams['JOB_FDCQXXB.FZDMJ'].length ==
						0) {
						Toast('请填写土地使用面积!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FJZMJ'] || this.valuesParams['JOB_FDCQXXB.FJZMJ'].length ==
						0) {
						Toast('请填写建筑面积!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FFWLX'] || this.valuesParams['JOB_FDCQXXB.FFWLX'].length ==
						0) {
						Toast('请选择房屋类型!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FZCS'] || this.valuesParams['JOB_FDCQXXB.FZCS'].length ==
						0) {
						Toast('请填写总层数!');
						return;
					} else if (!this.valuesParams['JOB_FDCQXXB.FZZC'] || this.valuesParams['JOB_FDCQXXB.FZZC'].length ==
						0) {
						Toast('请填写终止层!');
						return;
					}
					...
					if (_applicants.length == 0) {
						...
						for (var i = 0; i < this.applicants.length; i++) {
							if (!_applicants[i]['JOB_SQRXXB.FSQRMC'] || _applicants[i]['JOB_SQRXXB.FSQRMC'].length == 0) {
								...
								return;
							} else if (!_applicants[i]['JOB_SQRXXB.FZJZL'] || _applicants[i]['JOB_SQRXXB.FZJZL'].length ==
								0) {
								Toast('请选择申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '的证件种类!');
								return;
							} else if (!_applicants[i]['JOB_SQRXXB.FZJHM'] || _applicants[i]['JOB_SQRXXB.FZJHM'].length ==
								0) {
								Toast('请填写申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '的证件号码!');
								return;
							} else if (_applicants[i]['JOB_SQRXXB.FZJZL'] === "身份证" && !reg.test(_applicants[i][
									'JOB_SQRXXB.FZJHM'
								])) {
								Toast('请填写申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '正确的证件号码!');
								return;
							} else if (!_applicants[i]['JOB_SQRXXB.FDWXZ'] || _applicants[i]['JOB_SQRXXB.FDWXZ'].length ==
								0) {
								Toast('请选择申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '的单位性质!');
								return;
							} else if (!_applicants[i]['JOB_SQRXXB.FGJDQ'] || _applicants[i]['JOB_SQRXXB.FGJDQ'].length ==
								0) {
								Toast('请选择申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '的国家/地区!');
								return;
							} 
							/* else if (!_applicants[i]['JOB_SQRXXB.FHJSZSS'] || _applicants[i]['JOB_SQRXXB.FHJSZSS']
								.length == 0) {
								Toast('请选择申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '的户籍所属省份!');
								return;
							} */
							 else if (!_applicants[i]['JOB_SQRXXB.FLXDH'] || _applicants[i]['JOB_SQRXXB.FLXDH'].length ==
								0) {
								...
								return;
							} else if (!_applicants[i]['JOB_SQRXXB.FGYQK'] || _applicants[i]['JOB_SQRXXB.FGYQK'].length ==
								0) {
								Toast('请选择申请人' + _applicants[i]['JOB_SQRXXB.FSQRMC'] + '的共有情况!');
								return;
							} else if (!_applicants[i]['JOB_SQRXXB.FQLBL'] || _applicants[i]['JOB_SQRXXB.FQLBL'].length ==
								0) {
					...
					if (this.valuesParams['JOB_SJDJB.FSFKDJCL'] === '是' || this.valuesParams['JOB_SJDJB.FSFKDJZ'] ===
						...
						if (!this.valuesParams['JOB_SJDJB.FDXLXR'] || this.valuesParams['JOB_SJDJB.FDXLXR'].length == 0) {
							...
							return;
						} else if (!this.valuesParams['JOB_SJDJB.FDZ'] || this.valuesParams['JOB_SJDJB.FDZ'].length ==
							0) {
				...
				startExactBusiness: function (rid, businessNumber) {
					...
					this.$fetch(START_EXACT_BUSINNESS, {
							...
							//土地性质
							this.valuesParams['JOB_FDCQXXB.FQLXZ'] = response['JOB_GLQLXXB_LINK.OLD_IQLDJ'][0][
								'JOB_GLQLXXB.FQLXZ'
							];
							//土地用途
							this.valuesParams['JOB_FDCQXXB.FTDYT'] = response['JOB_GLQLXXB_LINK.OLD_IQLDJ'][0][
								'JOB_GLQLXXB.FYT'
							];
							//土地使用面积
							this.valuesParams['JOB_FDCQXXB.FZDMJ'] = response['JOB_GLQLXXB_LINK.OLD_IQLDJ'][0][
								'JOB_GLQLXXB.FBDCDYMJ'
							];
							this.applicants = response['JOB_SQRXXB_LINK.IQLR'];
							Toast.clear();
							this.fillSubFormData('JOB_GLQLXXB_LINK.OLD_IQLDJ', response[
