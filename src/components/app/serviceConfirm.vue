  ...
  <div style="display:flex;flex-direction:column">
    ...
    <div class="search-div">
      <div class="Cdiv Cdiv-display">
        ...
        <el-button v-on:click="search" class="schequery" icon="el-icon-search"></el-button>
      </div>
			<div>
				<!--手机号码身份验证-->
				<div class="Cdiv">
				...
				<div class="Cdiv">
					...
					<el-button class="check-btn" :disabled="curCount!=0" @click="getVerificationCode">{{verificationCodeBtn}}</el-button>
				</div>
			</div>
    </div>
    <!-- 搜索栏 end-->
        ...
        if(_this.searchParam !== '') {
          ...
          this.$fetch('/formengineWebService/public/getConfigDataByJid?jid=' + _this.searchParam +
            ...
            if (response) {
              ...
              _this.$router.push({path: '/messageApprovenew', query: {token: _this.searchParam}});
            } else {
							Toast('请输入正确的验证码！');
            }
          }).catch( error => {
