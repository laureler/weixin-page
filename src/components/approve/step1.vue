<template>
	...
	<div class="step1">
		...
		<div class="wait_tips" v-show="isRead">识别中<br>请稍后</div>
		<div class="wait_tips" v-show="frontTips">认证失败,<br>请重新上传</div>
	</div>
</template>
    ...
    export default {
        ...
        methods: {
            onReadFront (file) {
                ...
                _this.$post('/pubWeb/public/faceRecognition/recognitionIdCard', formData, config).then(res => {
                    ...
                    if (res.err_no == 0) {
                        ...
                        if (_this.backEndShow == false) {
                            _this.isShow = true
                        }
                    } else {
                        //正面上传失败
                        _this.frontTips = true
                        setTimeout(function () {
                            _this.frontTips = false
                        }, 1500)
            ...
            next () {
                ...
                this.$router.push({ path: '/step2', query: { verified: this.isCheckBefore } })
        ...
        mounted () {
            ...
            if (openId) { // 判断微信用户是否已认证，如果已认证直接进入到人脸识别过程
                this.$fetch('/pubWeb/public/faceRecognition/isUserHaveAuthenticated?openId=' + openId).then(rs => {
                    // this.$fetch('http://www.aiwandoudou.com/pubWeb/public/faceRecognition/isUserHaveAuthenticated?openId='+openId).then(rs=>{
                    console.log(rs)
                    if (rs) {
                        this.next()
                    } else {
...
.step1{
	...
	.approve_card_front{
        ...
        .card_front_content{
            ...
            height:100%;
            position: relative;
            background-image: url('../../../public/images/approve/icon_shangchuang@2x.png');
    ...
    .show_img{
    	...
    	.deleteImg{
            ...
            border-radius: 100%;
            position: absolute;
            background-image: url('../../../public/images/approve/icon_shandiao@2x.png');
            background-repeat: no-repeat;
    ...
    .step1_notice{
        ...
        font-size: 26/2px;
        text-indent:46/2px;
        background-image: url('../../../public/images/approve/icon_tisi@2x.png');
        background-repeat: no-repeat;
