        ...
        _this.$post('/pubWeb/public/faceRecognition/setAuthenticationResult',formData,config).then(data=>{
          ...
          if(_this.$store.state.callbackUrl){
            ...
            if(_this.$store.state.callbackUrl == '/pubWeb/public/system/WeChatRemoteCheck') {
              ...
              _this.$router.push({
                path: '/step2new', query: {isSuccess:-10,cardCode:_this.data_id,username:_this.data_name}
              });
            }
            else {
            ...
            //默认验证成功页面
            _this.$router.push({
              path: '/step2new', query: {isSuccess:1,cardCode:_this.data_id,username:_this.data_name}
            });
          }
        }).catch(error=>{
          console.log(error);
          _this.$router.push({
            path: '/step2new', query: {isSuccess:0}
          });
        });
      },
      WeChatFaceCheck(){
